...
| **V** | Pacific vs Tethyan systems; ridge in a one-sided ocean gets subducted; the pulled continent then rifts, India-style tear or Pangea-style breakup; sutures/hotspots make it likelier. | **Spatially resolved intraplate stress** (replaces the per-plate tension clock): boundary tractions are coarsened to 250 km cells; every `stress_every` (2 Myr) each continental parcel gets a tension value — boundary pulls directed away from the parcel add (opposing pulls reinforce), minus `stress_beta` (0.6) of the net plate-moving force, with a `stress_l_km` (4000 km) influence length. **Stress relief**: lithosphere within 1500 km of a same-plate divergent boundary or recent rift is relaxed ×0.15 — the negative feedback without which each split raises its neighbours' tension and breakup cascades into a rift storm. Rifts nucleate where tension × (1 + weakness) exceeds `rift_threshold` (20), at `rift_rate` (0.04)/Myr, up to 3 simultaneous rifts per plate with ≥0.5 rad spacing — a trench-ringed supercontinent now fails through the corridors between opposing pulls, Tethyan single-sided pull yields ~40 % of the two-sided tension and fails near the pulled margin. Tips then propagate along weakness exactly as before. The field is rendered as the `stress` layer. | done |
...
| **VII** | Andean (net convergence) vs W-Pacific (rollback) margin style set by absolute motions. | Where the slab at a trench averages ≥ `rollback_age` (80 Myr) **and the upper plate is in tension behind the arc** (mean stress ≥ `backarc_stress` × `rift_threshold`; intra-oceanic arcs, which carry no stress field, pass on the age gate alone), the arc can **detach** (probability `rollback_rate`/Myr) as its own plate: the upper-plate sliver within `backarc_km` (300 km) of the trench. Slab suction then rolls it trenchward at a mantle-limited `rollback_v` (30 km/Myr) and a **back-arc basin** of new crust opens behind it. Young slabs never detach — Andean style. | done |
