...
fn main() {
    ...
    writeln!(log, "t,plates,parcels,cont_frac,mean_v,max_v,subducted,created,accreted,absorbed,cont_lost,cont_grown,rifts,merges").unwrap();

    let n_steps = (w.p.years / w.p.dt).round() as usize;
    let slice_steps = ((w.p.slice_every / w.p.dt).round() as usize).max(1);
    let t0 = Instant::now();
    for k in 0..=n_steps {
        if k % slice_steps == 0 || k == n_steps { render::render(&w); w.record_rotations(); }
        if k == n_steps { break; }
        step::step(&mut w);
        let s = &w.stats;
        writeln!(log, "{},{},{},{:.4},{:.2},{:.2},{},{},{},{},{},{},{},{}", w.t, s.n_plates, s.n_parcels, s.cont_frac, s.mean_v, s.max_v,
            s.subducted, s.created, s.accreted, s.absorbed, s.cont_lost, s.cont_grown, s.rifts, s.merges).unwrap();
